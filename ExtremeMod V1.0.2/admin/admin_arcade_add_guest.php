...
if( !empty($setmodules) )
{
	...
	$module['Administration_Arcade_VPro']['Game_ajout_invités'] = $file;
	return;
}
...
$template->set_filenames(array(
	'body' => 'admin/admin_arcade_add_guest.tpl')
	);

$page_title = $lang['arcade_add_guest'];
$arcade_config = array();
...
if(! empty($mode))
{
  $selected_check = ( isset($HTTP_POST_VARS['select_add_game']) ) ?  $HTTP_POST_VARS['select_add_game'] : array();
	$select_id_sql = '';
	...
	if($tot >=1)
	{
   $sql = "UPDATE " . GAMES_TABLE . " SET game_auth_acc = 1
...
if ( $total = $db->sql_fetchrow($result) )
{
  ...
  $pagination = generate_pagination("admin_arcade_add_guest.$phpEx?sort=$sort_method&amp;order=$sort_order", $total_games, $games_par_page, $start). '&nbsp;';
}
  ...
  "S_ACTION" => append_sid("admin_arcade_add_guest.$phpEx?mode=$mode"))
  );

  // On affiche le bouton supprimer seulement s'il existe au moins un jeu dans la liste
  if ( $total_games>0 )
  {
    $template->assign_block_vars('switch_listeadd_existante', array());
  }
  ...
  {
	$template->assign_block_vars('listeadd', array(
