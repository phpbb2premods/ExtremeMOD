...
* @version $Id: admin_quick_title.php,v 1.6.6 2007/11/06 22:26 OxyGen Powered Exp $
		...
		*/
		...
		if ($mode == 'edit')
		{
			...
			$template->assign_vars(array(
				'L_TITLE'					=> sprintf($lang['Attribute_Edit'], $attribute['attribute_color'], ((isset($lang[$attribute['attribute']])) ? $lang[$attribute['attribute']] : $attribute['attribute'])),
	...
	else if ($mode == 'save')
	{
		...
		*/
		if ( !empty($color) )
		{
			...
			{
				message_die(GENERAL_MESSAGE, '<br />' . $lang['Attr_Error_Message_08'] . $lang['Attribute_Color']);
			}
		}
	...
	*/
	...
	for ( $i = 0; $i < $row_count; $i++ )
	{
		...
		$template->assign_block_vars('attribute', array(
			...
			'ATTRIBUTE'		=> !$row[$i]['attribute_type'] ? '<strong style="color:#' . $color . '">' . $text  . '</strong>' : $image,
			'COLOR'				=> $row[$i]['attribute_color'] ? '<strong style="color:#' . $color . '">' . $color  . '</strong>' : $lang['Attribute_None'],
