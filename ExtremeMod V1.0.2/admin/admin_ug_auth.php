...
//
$forum_auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_delete', 'auth_sticky', 'auth_global_announce', 'auth_announce', 'auth_vote', 'auth_pollcreate');
...
else if ( ( $mode == 'user' && ( isset($HTTP_POST_VARS['username']) || $user_id ) ) || ( $mode == 'group' && $group_id ) )
{
	...
	*/
	...
	{
		$forum_id = $_sf_tree->data[$tree_id]['forum_id'];
		if ( $_sf_tree->data[$tree_id]['forum_type'] == POST_CAT_URL )
		{
			$nest_level[$tree_id] = 0;
			$i = 1;
			if ( empty($_sf_tree->data[$tree_id]['forum_id']) )
			{
				continue;
			}
		}
		else if ( isset($auth_ug[$forum_id]) )
		{
			$nest_level[$tree_id] = intval($nest_level[ $_sf_tree->make_id($_sf_tree->data[$tree_id]['forum_parent'], $_sf_tree->data[$tree_id]['cat_id']) ]) + 1;
			$user_ary = $auth_ug[$forum_id];
			$i = $_sf_tree->data[$tree_id]['idx_row'];
	//-- mod: sf - end
		if ( empty($adv) )
		{
		...
		else
		{
			...
			for($j = 0; $j < count($forum_access); $j++)
			{
				if ( $forum_access[$j]['forum_id'] == $forum_id )
				{
					...
					*/
					$j = $_sf_tree->data[$tree_id]['idx_row'];
					{{
					//-- mod: sf - end
		...
		//-- mod: sf
		$parent_id = $_sf_tree->make_id($_sf_tree->data[$tree_id]['forum_parent'], $_sf_tree->data[$tree_id]['cat_id']);
		if ( !intval($_sf_tree->data[$tree_id]['forum_parent']) && ($_sf_tree->data[$tree_id]['idx'] == $_sf_tree->data[$parent_id]['idx'] + 1) )
		{
			// send a cat row
			$template->assign_block_vars('forums.catrow', array(
				'SPAN' => ($adv ? count($forum_auth_fields) : 1) + 2,
				'CAT_TITLE' => $_sf_tree->data[$parent_id]['forum_name'],
			));
		}
		//-- mod: sf - end
