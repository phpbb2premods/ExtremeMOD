...
if ( $board_config['shoutbox_on'] && $shoutbox != "off" && !$shoutbox_banned_view ) if ( $board_config['shoutbox_allow_guest'] || $board_config['shoutbox_allow_guest_view'] || $userdata['session_logged_in'] )
{
	...
	$number = $board_config['shoutbox_count_msg'];
...
//-- add
	$sql = 'SELECT sb.id, sb.sb_user_id, sb.msg, sb.timestamp, sb.sb_username, sb.shout_bbcode_uid, sb.enable_bbcode, sb.enable_html, sb.enable_smilies, u.user_level, u.user_color, u.user_group_id
		...
		ORDER by timestamp DESC LIMIT ' . $start . ', ' . NUM_SHOUT;
...
//-- fin mod : rank color system -----------------------------------------------
			...
			$name = ( $board_config['shoutbox_links_names'] ) ? '<a href="' . append_sid('profile.'.$phpEx . '?mode=viewprofile&u=' . $name_id) . '" target="_top" class="gensmall" title="' . $lang['Read_profile'] . '">' . $username . '</a>' : $username;
			...
			if ( $aedit )
			{
				...
				$editmsg = '<a href="' . $temp_url_e . '" class="mainmenu"><img src="' . $images['icon_edit_shout'] . '"></a>';
			}
			else
			{
				$editmsg = '';
