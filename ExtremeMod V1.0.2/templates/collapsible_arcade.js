...
{
	this.games[this.games.length] = f;
}
//
...
_CFIG.prototype.slideGames = function(cat_id)
{
	...
	for( var i=0; i < oCategory.games.length; i++ )
	{
		this.appendStep(oCategory.games[i], newStatus);
		oCategory.status = newStatus;
	}
	if( newStatus == '' ) this.appendStep(catName, oldStatus);
	return newStatus;
}
_CFIG.prototype.restoreArcadeState = function(cookie_name)
{
	...
	for( catName in this.C )
	{
		this.slideGames(this.C[catName].cat_id);
	}
	this.execQueue();
}
_CFIG.prototype.saveArcadeState = function(cookie_name)
{
	var catName, state = '';
	for( catName in this.C )
	{
...
_CFIG.prototype.cmd = function(cmd)
{
	...
	case 'restoreState':
		...
		{
			this.restoreArcadeState(this.T['cookie']+'_s0');
			this.saveArcadeState(this.T['cookie']);
		}
		break;
	case 'deleteState':
		...
		break;
	case 'saveState':
		this.saveArcadeState(this.T['cookie']+'_s0');
		break;
	case 'expandAll':
		this.delCookie(this.T['cookie']);
		this.restoreArcadeState(this.T['cookie']);
		break;
	case 'collapseAll':
		for( catName in this.C )
		{
...
_CFIG.prototype.writePanel = function()
{
	...
	s += '<table width="99%" cellpadding="4" cellspacing="0" align="center" border="0" class="forumline"><tr><td class="row1" align="center">';
	s += '<table width="99%" cellpadding="0" cellspacing="0" align="center" border="0">';
	...
	s += this.genRow(i++)+'&nbsp;</td></tr>';
	s += this.genRow(i++)+'&nbsp;</td></tr>';
	s += this.genRow(i++)+'<span class="copyright">'+CFIG_Version+' &copy; 2004 by <a href="http://www.phpmix.com" target="_blank" name="phpmix" class="copyright">phpMiX</a> And Modified by <a href="http://www.foruminfo.performant-hosting.com/2/portal.php" target="_blank">Polo</span></td></tr>';
