	...
	if ( (!empty($HTTP_POST_VARS['vote_id'])) && (is_array($HTTP_POST_VARS['vote_id'])) && (isset($HTTP_POST_VARS['vote_id'])) )
	{
		...
		if ( $vote_info = $db->sql_fetchrow($result) )
		{
			...
			if ( $max_vote >= (count($vbn) + $prev_vote) )	//	Modded	
			{
				...
				$sql = "INSERT INTO " . VOTE_USERS_TABLE . " (vote_id, vote_user_id, vote_option_id, vote_user_ip, vote_cast) 
					VALUES ($vote_id, " . $userdata['user_id'] . ", '$user_ip', $vote_option_id, " . $userdata['user_id'] . ")";
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			//-- mod : quick title edition -------------------------------------------------
				...
				//	, $attribute_id
				//-- modify
				submit_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $username), str_replace("\'", "''", $subject), $sub_title, str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length, $added, $max_vote, $hide_vote, $undo_vote, $attribute_id);
				//-- fin mod : quick title edition ---------------------------------------------
			}
			break;
